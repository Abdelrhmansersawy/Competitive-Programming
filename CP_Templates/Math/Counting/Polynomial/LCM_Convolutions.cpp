...
Calculate integer sequence c1,...,cN as follows:
    ...
    c_k = sum_{ lcm(i, j) = k } a_i * b_j  (mod 998244353)
...
Time complexity:
    ...
    - Divisor Zeta / Mobius transforms:
...
Problem link: https://judge.yosupo.jp/problem/lcm_convolution
*/
...
void DivisorZetaTransform(vector<mod>& v) {
    const int n = (int)v.size() - 1;
    for (int p : PrimeEnumerate(n)) {
        for (int i = 1; i * p <= n; i++)
            v[i * p] = add_mod(v[i * p], v[i]);
    }
}

void DivisorMobiusTransform(vector<mod>& v) {
    const int n = (int)v.size() - 1;
    for (int p : PrimeEnumerate(n)) {
        for (int i = n / p; i >= 1; i--)
            v[i * p] = sub_mod(v[i * p], v[i]);
    }
}

vector<mod> LCMConvolution(vector<mod> A, vector<mod> B) {
